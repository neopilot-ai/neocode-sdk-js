...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['NEOCODEE_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['NEOCODEE_LOG'] or 'warn' if it isn't set.
   */
...
export class Neocode {
   ...
   * API Client for interfacing with the Neocode API.
   *
   * @param {string} [opts.baseURL=process.env['NEOCODEE_BASE_URL'] ?? http://localhost:54321] - Override the default base URL for the API.
   ...
   */
  constructor({ baseURL = readEnv('NEOCODEE_BASE_URL'), ...opts }: ClientOptions = {}) {
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('NEOCODEE_LOG'), "process.env['NEOCODEE_LOG']", this) ??
      defaultLogLevel;
